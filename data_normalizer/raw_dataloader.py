...
def _get_roi_ts(path, parcel, nw_info, args):
    ...
    roi_ts = np.zeros((t, args.voxel))
    for ii in range(args.voxel):
        roi_ts[:, ii] = z_ts[:, ii]
...
def run(args):
    ...
    for ii, ID in enumerate(participants):
        ...
        if len(ID_files) == 4:
            ...
            '''
            save_ts = np.zeros((k_time, args.voxel, 4))
            for run in range(4):
                run_ts = ID_ts[run]
                t = run_ts.shape[0]
...
# todo: refactor parse  argument to function params
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--voxel', type=int,
                        default=59412, help='number of ROI')
